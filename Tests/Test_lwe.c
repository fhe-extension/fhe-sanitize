...
/*	for (int i = 0; i < _lwe_size1-1; ++i)
	...
	lwe_sk ss =	lwe_gaussian_keygen(LWE_SIZE_PRIME, powl(2.0L,1.2L));//cle ancienne
	...
	if (TEST_ENC_DEC) 
	{
		for(int i = 0; i < n_test; ++i) 
		{
			...
			start_chrono();
			lwe_encrypt_over(ct, skenc, lwe_M, (1L<<59)/lwe_M, m, _lwe_size1);
...
/*			for (int i = 0; i < _lwe_size1; ++i)
			...
			start_chrono();
                        double err;
			m_ = lwe_decrypt_and_keep(skenc, lwe_M, ct, _lwe_size1, &err);
                        accum_var += err*err;
